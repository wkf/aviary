(ns aviary-www.dev
  (:require [aviary-www.style :as style]
            [aviary-www.markup :as markup]
  ...
  :watch (using [fw :figwheel]
           (watch [(w/watch-clj
                     ["src/dev/clj"
                      "src/main/clj"]
                     #(condp = %
                        'aviary-www.style (->> (style-manifest) keys (fw/reload-css fw))
                        'aviary-www.markup (->> (markup-manifest) keys (fw/reload-html fw)) nil))
                   (fw/watch-cljs
                     fw
